...
def test_distribution_metrics_separate_sharp_and_flat_rows() -> None:
    sharp, flat = summarize_distribution_rows(
        np.array([[0.0, 0.99, 0.01], [1 / 3, 1 / 3, 1 / 3]])
    )

    assert sharp["argmax_index"] == 1
    assert sharp["distribution_kind"] == "probabilities"
    assert sharp["max_probability"] == pytest.approx(0.99)
    assert sharp["top_two_probability_margin"] == pytest.approx(0.98)
