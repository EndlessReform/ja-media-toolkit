...
class RunHistoryReader:
    ...
    def list(self, *, offset: int, limit: int) -> RunPage:
        ...
        return RunPage(
            items=tuple(self._summary(item) for item in facts), total=total,
    ...
    def _summary(self, run: RunFact) -> RunSummary:
        ...
        return RunSummary(
            run_id=run.run_id, run_number=run.run_number, target=run.job_name,
            status=run.status, started_at=run.started_at, finished_at=run.finished_at,
            terminal_snapshot_id=run.snapshot_id, duration_ms=duration,
