...
from ja_media_data.products.subtitle_lid.compiler import (
    ...
    compile_product as compile_lid_product,
)
from ja_media_data.products.subtitle_lid.repository import (
    replace_product as replace_lid_product,
)
...
def subtitle_lid(context) -> dg.MaterializeResult:
    """Classify every subtitle in the current canonical-input product."""

    runtime = _runtime(context)
    repository = runtime.product_repository()
    heads = input_heads(repository, "canonical_inputs")
    product = compile_lid_product(repository.connection, runtime.store)
    committed = replace_lid_product(
        repository.connection,
        product,
        commit_context(
            context,
