...
ranges. Re-seeking compressed remote media for every cue is unpleasant, so this
module decodes once and retains compact mono ``int16`` PCM in memory.

Playback deliberately uses sounddevice's convenience API. It already owns the
PortAudio callback and interruption lifecycle; callers should not grow their
own output threads merely to stop one range and start another.
"""
...
class MaterializedAudio:
    ...
    def slice_samples(self, start_s: float, duration_s: float) -> NDArray[np.int16]:
...
def materialize_audio(
    ...
    *,
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"Could not decode first audio stream with ffmpeg:\n{detail}")
