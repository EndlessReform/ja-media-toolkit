"""Resolve and cache indexed anime audio for interactive local playback."""
...
def resolve_subsync_audio(
    ...
    *,
    ...
    try:
        ...
        artifact = audio_client.artifact(anilist_id, episode_key, profile=profile)
        ...
        return _fallback_or_raise(
            promotion_target, f"Derived audio service unavailable: {exc}"
        )
...
def _validated_source(source: Path | None) -> Path | None:
    ...
    if not resolved.is_file():
        raise ValueError(f"playback source is not a file: {resolved}")
    return resolved


def _fallback_or_raise(
    promotion_target: Path | None, reason: str
) -> SubsyncAudioSelection:
    if promotion_target is None:
...
def _cached_artifact(
    ...
    *,
    ...
    content = client.content(
        artifact.anilist_id, artifact.episode_key, profile=artifact.profile
    )
