...
from ja_media_frontend.subsync.service import (
    discover_subtitle_file as discover_shared_subtitle_file,
    load_subtitle_track,
)
...
def discover_subtitle_file(media_path: Path, *, sub_file: str | Path | None) -> Path:
    """Compatibility wrapper around shared SRT/ASS sidecar discovery."""

    try:
        return discover_shared_subtitle_file(media_path, sub_file=sub_file)
    except ValueError as exc:
