...
def load_manifest(path: Path) -> AnimeAudioManifest:
    ...
    if payload.get("schema_version") != 1 or payload.get("kind") != "anime-audio-series":
        raise ValueError(f"unsupported anime audio manifest: {path}")
    ...
    episodes = tuple(_episode_from_mapping(item) for item in payload.get("episodes", []))
    return AnimeAudioManifest(
        series=series,
        profile=AudioProfile(**payload["profile"]),
        episodes=episodes,
        cover=CoverArtifact(**cover_payload) if cover_payload else None,
    )
...
def manifest_to_mapping(manifest: AnimeAudioManifest) -> dict[str, object]:
    """Convert the typed manifest to its stable on-disk representation."""
...
def _episode_from_mapping(payload: dict[str, Any]) -> ManifestEpisode:
